...
local function initPopulation()
    population = {}
    for s = 1, NUM_SEQUENCES do population[s] = randomSequence() end
end
...
local function mutate(seq, evolveRate)
    ...
    for i = 1, STEPS_PER_SEQ do
        if math.random() < mutateProbability then
            ...
            if MUTATE_GATES then seq.gates[i] = not seq.gates[i] end
        end
    end
end
...
local function computeFitness(seq)
    ...
    for i = 1, STEPS_PER_SEQ do
        ...
        if i > 1 then
            ...
            if interval < 0.0001 then score = score - 0.05 end
        end
    end

    -- variety
    local distinctCount = 0
    for _ in pairs(uniqueVals) do distinctCount = distinctCount + 1 end
    ...
    local gateOnCount = 0
    for i = 1, STEPS_PER_SEQ do
        if seq.gates[i] then gateOnCount = gateOnCount + 1 end
    end
...
local function pickParent()
    local sumF = 0
    for s = 1, NUM_SEQUENCES do sumF = sumF + computeFitness(population[s]) end
    local r = randFloat(0, sumF)
    local cum = 0
    for s = 1, NUM_SEQUENCES do
        cum = cum + computeFitness(population[s])
        if r <= cum then return population[s] end
    end
...
return {
    ...
    -- Standard pot usage
    pot1Turn = function(self, val) standardPot1Turn(val) end,

    pot2Turn = function(self, val) standardPot2Turn(val) end,

    pot3Turn = function(self, val) standardPot3Turn(val) end,
    ...
    draw = function(self)
        ...
        drawText(10, yOffset, "Seq " ..
                     string.format("%d", currentSequenceIndex) .. "/" ..
                     NUM_SEQUENCES)
        drawText(110, yOffset, "Evolutions: " .. evolutionCount)
        drawText(10, yOffset + 10,
                 "Fitness: " .. string.format("%.2f", seq.fitness))
