...
return {
    ...
    init = function(self)
        return {
            ...
            inputNames = {"Clock", "Reset", "Arm", "Stop"},
            outputNames = {"Takeover Gate"},
            parameters = {{"Bars", 1, 16, 4, kInteger}}
        }
    end,
    ...
    trigger = function(self, input)
        if input == 2 then
            -- Reset
            pendingArm = false
            if recording then setParameter(looper, recordParam, 0) end
            recording = false
            cue = false
            stopRecordingTime = -1
        elseif input == 3 then
            -- Arm
            if not recording then pendingArm = true end
        elseif input == 4 then
