...
describe('Display designer dialog', () => {
  ...
  it('opens from the utility command, edits every primitive type, and updates the raster source', async () => {
    ...
    expect(document.body.textContent).toContain('64 pixels · 1 optimized draw call')
    await click(button('Paint shade 7'))
    await click(button('Pixel 1, 1: shade 15'))
    expect(button('Pixel 1, 1: shade 7')).not.toBeNull()
    expect(document.body.textContent).toContain('64 pixels · 2 optimized draw calls')
    ...
    expect(document.body.textContent).toContain('Primitive elements10')
    expect(document.body.textContent).toContain('Visible draw calls14')
