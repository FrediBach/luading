...
describe('Display designer rendering', () => {
  it('renders a labelled full-size authoring dialog with every static primitive path', () => {
    ...
    for (const label of ['Select', 'Pixel line', 'Smooth line', 'Animated line', 'Outline box', 'Filled box', 'Pixel box', 'Pixel circle', 'Smooth circle', 'Polygon', 'Bézier curve', 'Standard text', 'Tiny text']) {
    ...
    expect(markup).toContain('role="tablist" aria-label="Display designer left panel"')
    expect(markup).toContain('id="display-designer-left-tab-layers"')
    expect(markup).toContain('aria-selected="true" aria-controls="display-designer-left-panel-layers" tabindex="0"')
    expect(markup).toContain('role="tabpanel" id="display-designer-left-panel-components" aria-labelledby="display-designer-left-tab-components" hidden=""')
    expect(markup).toContain('Search components')
    expect(markup).toContain('Component category')
    expect(markup).toContain('Input jack 1x pixel preview')
    expect(markup).toContain('Input jack magnified pixel preview')
    expect(markup).toContain('Integration disclosure')
    expect(markup).toContain('Insert Drum voice glyph')
  ...
  it('pins the desktop regions, collapsible columns, pixel raster, and responsive fill shell', () => {
    ...
    expect(designerCss).toMatch(/\.display-designer-sidebar--layers \{[^}]*display: grid;[^}]*grid-template-rows: auto auto minmax\(0, 1fr\);[^}]*overflow: hidden;/s)
    expect(designerCss).toMatch(/\.display-designer-left-tabs button\[aria-selected='true'\] \{[^}]*border-color: var\(--nt-green\);/s)
    expect(designerCss).toMatch(/\.display-component-card > header strong \{[^}]*font: 700 var\(--font-label\)\/1\.2 var\(--mono\);/s)
    expect(designerCss).toMatch(/\.display-component-card canvas \{[^}]*image-rendering: pixelated;/s)
  ...
  it('server-renders linked, roving panel tabs for medium and narrow layouts', () => {
    ...
    expect(medium).toContain('id="display-designer-tab-components"')
    ...
    expect(medium).toContain('id="display-designer-panel-components" aria-labelledby="display-designer-tab-components" hidden=""')
