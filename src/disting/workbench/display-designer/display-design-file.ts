...
export interface ParsedDisplayDesign {
  ...
  findings: DisplayDesignerFinding[]
  bytes: number
  migratedFromVersion?: 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8
}
...
export function parseDisplayDesignText(text: string): ParseDisplayDesignResult {
  try {
    ...
    const migratedFromVersion = typeof value === 'object' && value !== null
      && 'version' in value && (value.version === 1 || value.version === 2 || value.version === 3 || value.version === 4 || value.version === 5 || value.version === 6 || value.version === 7 || value.version === 8)
      ? value.version
      : undefined
    const validation = validateDisplayDesign(value)
    if (!validation.ok || !validation.document) {
