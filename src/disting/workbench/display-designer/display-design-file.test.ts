...
function legacyDocument(document: object, version: 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8): Record<string, unknown> {
  ...
  legacy.elements = (legacy.elements as Array<Record<string, unknown>>).map((element) => {
    ...
    if (copy.kind === 'pixel-box' && version < 8) {
...
`)
  ...
  it('migrates version-1 files without making them invalid and always serializes version 9', () => {
    ...
    expect(JSON.parse(serialized.text)).toMatchObject({ version: 9, tokens: [], layoutGrid: null, screens: [{ name: 'Screen 1' }] })
  })
  ...
  it('migrates version-4 pixel boxes and writes canonical version-9 polygons', () => {
  ...
  it('migrates version-5 polygons and writes canonical version-9 Bézier curves', () => {
  ...
  it('migrates version-7 static pixel boxes into one-frame version-9 boxes', () => {
  ...
  it('migrates version-8 animated pixel boxes without losing frames', () => {
    const ids = createSequentialDisplayDesignIdFactory('v8')
    const pixelBox = createDefaultDisplayPrimitive('pixel-box', ids)
    pixelBox.frameRate = 15
    pixelBox.frames = [pixelBox.frames[0]!, { ...structuredClone(pixelBox.frames[0]!), duration: 2 }]
    const legacy = legacyDocument({ ...createEmptyDisplayDesign('Version 8'), elements: [{ ...pixelBox, screenId: 'display-screen-1' }] }, 8)
    const parsed = parseDisplayDesignText(JSON.stringify(legacy))

    expect(parsed.ok).toBe(true)
    if (!parsed.ok) return
    expect(parsed.migratedFromVersion).toBe(8)
    expect(parsed.document.version).toBe(9)
    expect(parsed.document.elements[0]).toMatchObject({ kind: 'pixel-box', frameRate: 15, frames: [{ duration: 1 }, { duration: 2 }] })
  })
  ...
  it('pins canonical version-9 root, token, and AST key order', () => {
  ...
  it('rejects malformed, oversized, unknown-version, and invalid documents without throwing', () => {
    ...
    expect(parseDisplayDesignText(JSON.stringify({ ...createEmptyDisplayDesign(), version: 10 }))).toMatchObject({
      ok: false,
