...
- `signal-sources.ts` implements deterministic CV, gate, trigger, sequencer,
  ...
  editable gate and note-sequencer patterns, and browser-agnostic external held
  values and queued pulses. Sequencer patterns and Freeform points are
  browser-local input configuration, reuse the existing cycle timing and phase
  model, and do not extend the Lua contract.
