...
- `signal-sources.ts` implements deterministic CV, gate, trigger, sequencer,
  ...
  editable gate and note-sequencer patterns, configurable arpeggio patterns,
  and browser-agnostic external held values and queued pulses. Sequencer
  patterns, arpeggio settings, and Freeform points are browser-local input
