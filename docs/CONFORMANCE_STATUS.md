...
| ID | Capability | Status and evidence | User-visible consequence | Implementation and tests | Hardware confirmation |
| --- | --- | --- | --- | --- | --- |
| DSP-01 | Integer primitives, standard/tiny text, clipping, shades, and parameter line | **Approximate (text faces)** — Manual 1.12 and emulator font choices | Documented integer drawing and text behavior produces deterministic 256x64 commands/rasterization with 16 shade values. | [`display-api.ts`](../src/disting/emulation/display-api.ts), [`display-renderer.ts`](../src/disting/emulation/display-renderer.ts), display/font/bounds tests | Pixelmix at 8px and Tom Thumb at 6px are monochrome approximations, not verified firmware glyphs. Compare glyphs, spacing, baselines, and fallback against hardware with a recorded firmware version. |
...
| LIB-04 | Bundled `ntlib` support library | **Simulator extension** — project library contract | The workbench automatically preloads the pure-Lua modules under `ntlib` and `ntlib.*`. Export embeds the runtime modules when source contains a literal `require 'ntlib'` or `require 'ntlib.*'`, producing one hardware-ready Lua file; non-ntlib exports remain unchanged. Monaco derives module/function/method completion, hover, and signatures from the bundled sources. This does not extend the firmware global API. | [`src/ntlib`](../src/ntlib), [`ntlib-modules.ts`](../src/disting/ntlib-modules.ts), [`ntlib.test.ts`](../src/ntlib/ntlib.test.ts), [`script-file.test.ts`](../src/disting/workbench/script-file.test.ts), [`ntlib-intellisense.test.ts`](../src/disting/editor/ntlib-intellisense.test.ts) | Library arithmetic, deterministic state, standalone export loading, and editor-catalog completeness are automated; control feel, timing headroom, display appearance, and export execution on named firmware still require hardware validation. |
