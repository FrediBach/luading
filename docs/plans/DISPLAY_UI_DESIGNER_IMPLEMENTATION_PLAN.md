...
| 8. Responsive/accessibility polish and documentation | Implemented (2026-08-06); live browser verification pending | Focused layout/render/dialog coverage plus `npm test` (130 files, 752 tests); `npm run check` passed with 96.82% statements, 91.18% branches, 100% functions, and 98.43% lines. Pure/server-render/jsdom coverage pins the 900/720 px branches, Fit zoom, linked roving panel and symbol-state tabs, accessible dynamic controls/swatches/findings/metrics/announcements, touch targets, reduced motion, disclosures, file/source-handoff regressions, the themed portal host, declaration of every referenced CSS variable, and keyboard/right-click contextual actions for selected and non-selected layers, groups, symbols, and states. Live browser verification was attempted through the in-app browser, but the browser registry was empty. Wide desktop, 721-900 px, <=720 px, coarse pointer, keyboard-only navigation, reduced motion, browser zoom, largest supported text size, real focus rings, overflow/scrolling, and Canvas rendering remain unverified. Hardware checks are not applicable to this browser-only polish increment; the compiler, generated Lua, display API, worker protocol, and conformance support claims were unchanged. |
...
[SVG import plan](../archive/implementation-plans/DISPLAY_DESIGNER_SVG_IMPORT_PLAN.md) records conversion
into editable designer primitives and native text, with guided loss/size review.
The implemented follow-up supersedes the SVG deferral; the original
