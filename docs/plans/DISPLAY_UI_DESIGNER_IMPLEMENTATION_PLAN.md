...
| 8. Responsive/accessibility polish and documentation | Implemented (2026-08-06); live browser verification pending | `npx vitest run src/disting/workbench/display-designer/display-designer-layout.test.ts src/disting/workbench/display-designer/display-designer-rendering.test.tsx src/disting/workbench/display-designer/display-designer-dialog.test.tsx` (3 files, 21 tests); broader designer/renderer regression run (15 files, 104 tests); `npm test` (130 files, 750 tests); `npm run check` passed with 96.82% statements, 91.18% branches, 100% functions, and 98.43% lines. Pure/server-render/jsdom coverage pins the 900/720 px branches, Fit zoom, linked roving panel and symbol-state tabs, accessible dynamic controls/swatches/findings/metrics/announcements, touch targets, reduced motion, disclosures, and file/source-handoff regressions. Live browser verification was attempted through the in-app browser, but the browser registry was empty. Wide desktop, 721-900 px, <=720 px, coarse pointer, keyboard-only navigation, reduced motion, browser zoom, largest supported text size, real focus rings, overflow/scrolling, and Canvas rendering remain unverified. Hardware checks are not applicable to this browser-only polish increment; the compiler, generated Lua, display API, worker protocol, and conformance support claims were unchanged. |
