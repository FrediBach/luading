...
| 3. Dialog shell, entry point, and static-property editing | Complete (2026-08-06) | `npx vitest run src/disting/workbench/display-designer/display-designer-rendering.test.tsx src/disting/workbench/display-designer/display-designer-dialog.test.tsx src/disting/workbench/command-bar-rendering.test.tsx` (3 files, 9 tests); broader designer/renderer regression run (10 files, 57 tests); `npm test` (125 files, 707 tests); `npm run check` passed with 96.82% statements, 91.18% branches, 100% functions, and 98.43% lines. Live browser verification was unavailable because no controllable browser backend was registered; CSS layout, native focus behavior, and real Canvas rendering remain unverified. Hardware checks are not applicable to this browser-only authoring increment; the renderer and generated Lua contract were unchanged. |
| 4. Direct manipulation, layers, and keyboard workflow | Implemented (2026-08-06); live browser verification pending | `npx vitest run src/disting/workbench/display-designer/display-design-geometry.test.ts src/disting/workbench/display-designer/display-designer-dialog.test.tsx src/disting/workbench/display-designer/display-designer-rendering.test.tsx` (3 files, 15 tests); `npm test` (126 files, 717 tests); `npm run check` passed with 96.82% statements, 91.18% branches, 100% functions, and 98.43% lines. Pure tests cover transforms, snapping, constraints, bounds, hit testing, handles, movement, alignment/distribution, and layer ordering; jsdom covers pointer/keyboard transaction wiring without claiming browser layout or pointer-capture fidelity. Live browser validation was attempted through the in-app browser, but no browser backend was registered; drag creation, real pointer capture/cancellation, handle sizing, scroll/zoom interaction, keyboard-only focus visibility, touch behavior, and Canvas rendering remain pending. Hardware checks are not applicable to this browser-only authoring increment; generated Lua and the firmware-facing renderer contract were unchanged. |
| 5. Dynamic bindings and state preview | Implemented (2026-08-06); live browser verification pending | `npx vitest run src/disting/workbench/display-designer/display-design-bindings.test.ts src/disting/workbench/display-designer/display-design-validation.test.ts src/disting/workbench/display-designer/display-design-compiler.test.ts src/disting/workbench/display-designer/display-design-generator.test.ts src/disting/workbench/display-designer/display-designer-dialog.test.tsx src/disting/workbench/display-designer/display-designer-rendering.test.tsx` (6 files, 36 tests); broader designer run (9 files, 60 tests); `npm test` (127 files, 723 tests); `npm run check` passed with 96.82% statements, 91.18% branches, 100% functions, and 98.43% lines. Real Wasmoon cases matched compiler previews for number, boolean, and text bindings at multiple values. Live browser verification was attempted through the in-app browser, but no browser backend was registered; State-panel scrolling/layout, real focus rings, range/switch behavior, Canvas updates, touch/coarse-pointer interaction, and screen-reader output remain unverified. Hardware checks are not applicable to the browser-only binding controls; generated callbacks use the existing display API and the real Lua/display boundary was covered. |
| 6. Symbols, instances, and multi-state variants | Implemented (2026-08-06); live browser verification pending | `npx vitest run src/disting/workbench/display-designer src/disting/emulation/display-api.test.ts src/disting/workbench/script-scaffold.test.ts src/disting/workbench/LuaSourcePreview.test.tsx` (13 files, 87 tests); `npm test` (128 files, 733 tests); `npm run check` passed with 96.82% statements, 91.18% branches, 100% functions, and 98.43% lines. Real Wasmoon cases matched compiler expansion for literal and choice-bound states, multiple origins, shared helpers, unknown-state fallback, and repeated callbacks. Live browser verification was attempted through the in-app browser, but no browser backend was registered; real Canvas/layout, focus rings/return, pointer capture, state tabs/mapping, destructive confirmations, and keyboard-only symbol editing remain unverified. Hardware checks are not applicable to the browser-only authoring controls; generated helpers expand only to the existing display API, and the real Lua/display boundary was covered. |
| 7. Design-file portability and source handoff | Implemented (2026-08-06); live browser verification pending | `npx vitest run src/disting/workbench/display-designer src/disting/emulation/display-api.test.ts src/disting/workbench/script-scaffold.test.ts src/disting/workbench/LuaSourcePreview.test.tsx` (14 files, 97 tests); `npm test` (129 files, 743 tests); `npm run check` passed with 96.82% statements, 91.18% branches, 100% functions, and 98.43% lines. Pure tests pin canonical JSON bytes/order/newline, round trips, strict metadata/size/version validation, defensive parsing, and safe file names; jsdom covers non-destructive read/parse failures, discard-before-replace, collision-safe post-open editing, dispatched/failed download revisions, exact copy success, clipboard rejection, and selected manual-copy fallback. Live browser verification was attempted through the in-app browser, but no browser backend was registered; native file-picker filtering/cancellation, actual download creation, clipboard permissions, fallback selection, status announcements, focus behavior, and visual layout remain unverified. Project storage, backups, recovery, `.lua` import/export, Monaco, and the worker contract were unchanged. |
| 8. Responsive/accessibility polish and documentation | Implemented (2026-08-06); live browser verification pending | `npx vitest run src/disting/workbench/display-designer/display-designer-layout.test.ts src/disting/workbench/display-designer/display-designer-rendering.test.tsx src/disting/workbench/display-designer/display-designer-dialog.test.tsx` (3 files, 22 tests); broader designer/renderer regression coverage is included in `npm test` (130 files, 751 tests); `npm run check` passed with 96.82% statements, 91.18% branches, 100% functions, and 98.43% lines. Pure/server-render/jsdom coverage pins the 900/720 px branches, Fit zoom, linked roving panel and symbol-state tabs, accessible dynamic controls/swatches/findings/metrics/announcements, touch targets, reduced motion, disclosures, file/source-handoff regressions, the themed portal host, and declaration of every referenced CSS variable. Live browser verification was attempted through the in-app browser, but the browser registry was empty. Wide desktop, 721-900 px, <=720 px, coarse pointer, keyboard-only navigation, reduced motion, browser zoom, largest supported text size, real focus rings, overflow/scrolling, and Canvas rendering remain unverified. Hardware checks are not applicable to this browser-only polish increment; the compiler, generated Lua, display API, worker protocol, and conformance support claims were unchanged. |
